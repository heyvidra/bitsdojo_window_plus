## 0.7.0
    - Dialog result channel and this platform's FIRST windowClosed delivery, both riding the child-exit watch that already reaps spawned window processes. Dialog spawns get a unique `BDW_RESULT_FILE` path (scrubbed from grandchild environments like the other BDW_* vars); the dialog's Dart writes its result there before closing, and the parent reads + deletes the file when the child exits, then invokes 'windowClosed' `{name, result}` into its own engine. Ceiling, stated where it matters: windowClosed here reports only windows THIS process spawned — one process per window means nobody has the global view Win/mac get for free.

## 0.6.0
    - Dialog support for `openNewWindow(modality:)`, cross-process edition — every window here is its own process, so this is deliberately the degraded version. The dialog's process receives `BDW_MODALITY` / `BDW_PARENT_XID` (both scrubbed from grandchild environments, like the rest of the BDW_* identity vars), marks itself `GDK_WINDOW_TYPE_HINT_DIALOG` on every backend, and on X11 writes `WM_TRANSIENT_FOR` plus the modal hint through plain GDK property calls — no new Xlib link. Modal makes the parent insensitive until the dialog's process exits; the existing child-exit watch re-enables it, counted per parent so several modal children can't re-enable it early. Focus return after a modal closes is left to the window manager: presenting the parent from the exit watch would steal focus from a just-reused named window, because "reuse" here is a short-lived forwarder process whose exit lands right after the reused window was presented. Ceilings: stacking only on X11; the WM can still raise the blocked parent (no cross-process grab exists); `modality` with an already-open named window costs a brief, self-correcting parent input-block flicker.
