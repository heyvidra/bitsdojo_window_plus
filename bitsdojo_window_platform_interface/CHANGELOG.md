## 0.7.0
    - `DesktopWindow.openNewWindow` is now CONCRETE, returns `Future<WindowRef>`, auto-names unnamed windows, and delegates to `BitsdojoWindowPlatform.instance` — the three per-platform copies of the same forwarding override are gone. `openDialog(... modal:)` / `closeWithResult(map)` and the native-UI methods (`showNativeAlert` / `showNativeConfirm` / `showNativeMenu`) join it as concrete members; `WindowModality` remains the wire vocabulary on `BitsdojoWindowPlatform.openNewWindow` only.
    - `WindowCloseHub` — one dispatch point for "a window closed": platform implementations call `notifyClosed(name, resultJson?)` and the hub fans out to the `windowClosed` stream, the pending `openDialog` future for that name, and the legacy `onWindowClosed` callback, so the three consumers cannot drift apart per platform. Re-registering a still-pending dialog name returns the SAME future — `openDialog` on an existing name focuses that dialog, so both callers are waiting on one window.
    - `BitsdojoWindowPlatform.setWindowResult(json)` with a `MethodChannel` default ('setWindowResult'); Linux overrides it in Dart to write the `BDW_RESULT_FILE` handed down by the parent process, keeping the child side of the cross-process result channel native-free. It returns a Future and `closeWithResult` AWAITS the native acknowledgment before requesting the close — the store message and the close request travel on different native queues, and a close that lands first broadcasts null to the awaiting `openDialog`. Observed on macOS (the round trip delivered null until sequenced), not hypothetical.
    - The base `setWindowTitleBarButton*` no-op bodies now `assert` against `capabilities` in debug builds — a silent no-op is documented behavior, not a surprise to debug.
    - **Removed:** the deprecated `visible` getter/setter.

## 0.6.0
    - `WindowModality { none, modeless, modal }` and a `modality` parameter on both `openNewWindow`s (`DesktopWindow` and `BitsdojoWindowPlatform`). It crosses the channel by NAME (`'modeless'`/`'modal'`, with the key absent for `none`) rather than by index, so the pinning test asserts the names: a rename is a wire break even though a reorder is not — the mirror image of the index-pinned enums beside it. Absent or unknown reads as `none` on the native side, so an older runner under a newer Dart layer opens a plain window instead of throwing.
