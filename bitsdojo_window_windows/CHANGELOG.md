## 0.7.0
    - Dialog result channel: 'setWindowResult' stores a JSON payload keyed by the calling window's HWND, and the windowClosed broadcast delivers `{name, result}` when the window actually closes — through both the `closeWindow(name)` path and a user-initiated destroy. The stored value is the LAST one set (a vetoed close keeps it for the close that finally lands) and is erased once broadcast, so a window reopened under the same name can never replay a stale result.

## 0.6.0
    - Dialog support for `openNewWindow(modality:)`. Ownership is assigned after creation via `GWLP_HWNDPARENT` — the `WindowFactory` ABI that runner apps implement cannot carry a parent, and changing it would break every runner in the wild. Modal disables the owner BEFORE the factory runs (the classic Win32 order, so no input slips into the parent during construction), re-asserts the disable after it returns (the factory pumps messages, and a sibling dialog closing mid-construction could otherwise re-enable the shared owner), and re-enables + re-foregrounds on destroy. `closeWindow(name)` re-enables before `DestroyWindow`, because destroying a modal window while its owner is disabled makes Windows activate some other application's window. Several modal dialogs can share one owner — it comes back when the last one goes — and a Dart-vetoed close keeps it disabled, since the restore only runs on actual destruction (WM_NCDESTROY).
