import 'dart:ffi';
import 'package:win32/win32.dart';
import 'package:ffi/ffi.dart';

const WM_BDW_ACTION = 0x7FFE;

const BDW_SETWINDOWPOS = 1;
const BDW_SETWINDOWTEXT = 2;
const BDW_FORCECHILDREFRESH = 3;

final class SWPParam extends Struct {
  @Int32()
  external int x, y, cx, cy, uFlags;
}

void setWindowPos(
    HWND hWnd, HWND hWndInsertAfter, int x, int y, int cx, int cy, int uFlags) {
  final param = calloc<SWPParam>();
  param.ref
    ..x = x
    ..y = y
    ..cx = cx
    ..cy = cy
    ..uFlags = uFlags;
  PostMessage(hWnd, WM_BDW_ACTION, WPARAM(BDW_SETWINDOWPOS), LPARAM(param.address));
}

final class SWTParam extends Struct {
  external Pointer<Utf16> text;
}

void setWindowText(HWND hWnd, String text) {
  final param = calloc<SWTParam>();
  param.ref.text = text.toNativeUtf16();
  PostMessage(hWnd, WM_BDW_ACTION, WPARAM(BDW_SETWINDOWTEXT), LPARAM(param.address));
}

void forceChildRefresh(HWND hWnd) {
  PostMessage(hWnd, WM_BDW_ACTION, WPARAM(BDW_FORCECHILDREFRESH), LPARAM(0));
}
